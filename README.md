...
为了简化冗长的 `--yaml` 路径输入，我们提供了 `run.py` 脚本。它不仅支持自动搜索对应的配置文件，还支持**交互式层级选择**和**细粒度逐用例执行**。

```bash
# 【新增】交互式执行 (不带参数)
# 终端会弹出层级菜单，支持逐层进入目录，选择具体 YAML 文件，或一键执行某个目录下的所有测试！
# 当选中某个 YAML 文件后，还可以继续选择是要执行该文件内的所有用例，还是通过多选 (如 2,3 或 2-4) 执行部分用例。
python run.py
...
> **💡 关于多用例执行的提示：**
> 当执行整个文件或目录下所有的测试时，`run.py` 会自动解析 YAML 提取出每一个单独的 `testTxxxx` 测试用例，并**逐一拆分执行**。
> 执行过程中终端会有清晰的分隔线（`====`、`####`、`----`）和进度提示，如果某一个用例失败，不会中断其他用例的执行。
