"""Migrated from Apifox case #7542846. Source folder: Consumer Purchase flow - resell and coupon and refund and ship and fee and tax."""
# Apifox Case ID: 7542846  (traceability only — not needed to run)
NAME = "_Consumer_4-3_Mixed_items_enabled_my_items_resell_promoter_retail"
...
# pre: var transaction_fee_item_level = Math.floor(get_transaction_fee_item_level_general(200,1)*100)/100;
# pre: var merchant_transaction_fee = Math.round(((0/200 * transaction_fee_item_level) + Number.EPSILON) *100 ) / 100;
# pre: var promoter_transaction_fee = Math.round(((125/200 * transaction_fee_item_level) + Number.EPSILON) *100 ) / 100;
...
# post:     pm.response.to.have.status(201);
# post:     var res = JSON.parse(responseBody);
# post:     pm.expect(res['data']['totalTopay']).to.eql(409.98);
# post: });
# --- step 5: Login - P1 ---
...
# post: });
# --- step 9: get merhcant order detail v2 ---
...
# post:             pm.expect(line_item["lineItemPrice"]).to.eql(150);
# post:             pm.expect(line_item["retailPrice"]).to.eql(150);
...
# post:             pm.expect(line_item["commissionGross"]).to.eql(75);
# post:             pm.expect(line_item["commissionDeduction"]).to.eql(75);
# post:             pm.expect(line_item["costPrice"]).to.eql(75);
# post:             pm.expect(line_item["lineItemSubtotal"]).to.eql(150);                     
# post:             pm.expect(line_item["downlineCommissionRate"]).to.eql(62.5);
...
# post: 
# post: });
# --- step 10: get merchant commission charge ---
# pre: function get_transaction_fee_item_level_general(unit_price,quantity){
# pre:     unit_fixed_fee = 0;
# pre:     item_percentage_fee = 0.085;
# pre:     if (unit_price > 0){
...
# pre: var order_level_transaction_fee = Math.floor((4.99 * 0.085)*100)/100;
# pre: var transaction_fee_item_level = Math.floor(get_transaction_fee_item_level_general(200,1)*100)/100;
# pre: var promoter_transaction_fee2 = Math.round(((50/200 * transaction_fee_item_level) + Number.EPSILON) *100 ) / 100;
# pre: var merchant_transaction_fee2 = Number((transaction_fee_item_level + order_level_transaction_fee - promoter_transaction_fee2).toFixed(2));
...
# post:     pm.expect(data["totalPayoutToPay"]).to.eql(total_payout);
# post:     pm.expect(data["totalCommissionSubtotal"]).to.eql(150);
# post:     pm.expect(data["totalCommissionRefunded"]).to.eql(0);
# post:     pm.expect(data["totalCommissionNet"]).to.eql(150);
# post:     pm.expect(data["transactionFeeRebate"]).to.eql(0);
# post:     pm.expect(data["totalPaymentProcessingFee"]).to.eql(0);
# post:     pm.expect(data["totalShippingSubtotal"]).to.eql(4.99);
# post:     pm.expect(data["totalShippingRefunded"]).to.eql(0);
...
# post:     var total_payout = Number((res['data']['totalCommissionNet'] - promoter_transaction_fee - promoter_transaction_fee2).toFixed(2));
# post:     pm.expect(res['data']['totalPayoutToPay']).to.eql(total_payout);
# post:     pm.expect(res['data']['transactionFee']).to.eql(promoter_transaction_fee + promoter_transaction_fee2);
# post:     pm.expect(res['data']['totalCommissionSubtotal']).to.eql(175);
# post:     pm.expect(res['data']['totalCommissionRefunded']).to.eql(0);
# post:     pm.expect(res['data']['totalCommissionNet']).to.eql(175);
# post:     pm.expect(res['data']['totalPaymentProcessingFee']).to.eql(0);
# post:     pm.expect(res['data']['transactionFeeRebate']).to.eql(0);
# post:     var line_item_list = res['data']['lineItems'];
# post:     for (var line_item of line_item_list){
# post:            if (line_item["variantInfo"]["id"] === "e3a58292-0a2e-41ac-9d65-27cc6ea3ad27"){
# post:             pm.expect(line_item["lineItemPrice"]).to.eql(200);
# post:             pm.expect(line_item["retailPrice"]).to.eql(200);
# post:             pm.expect(line_item["upstreamLineItemDiscount"]).to.eql(0);
# post:             pm.expect(line_item["couponDiscount"]).to.eql(0);
# post:             pm.expect(line_item["subscriptionDiscount"]).to.eql(0);
# post:             pm.expect(line_item["lineItemDiscount"]).to.eql(0);
# post:             pm.expect(line_item["commissionRate"]).to.eql(62.5);
# post:             pm.expect(line_item["commissionGross"]).to.eql(125);
# post:             pm.expect(line_item["commissionDeduction"]).to.eql(0);
# post:             pm.expect(line_item["costPrice"]).to.eql(75);
# post:             pm.expect(line_item["lineItemSubtotal"]).to.eql(200);                     
# post:             pm.expect(line_item["downlineCommissionRate"]).to.eql(0);
# post:             pm.expect(line_item["commissionSubtotal"]).to.eql(125);
# post:             pm.expect(line_item["commissionRefunded"]).to.eql(0);
# post:             pm.expect(line_item["commissionNet"]).to.eql(125);          
# post:             pm.expect(line_item["quantity"]).to.eql(1);
# post:             pm.expect(line_item["quantityFulfilled"]).to.eql(0);
# post:             pm.expect(line_item["quantityRefunded"]).to.eql(0);
# post:             pm.expect(line_item["lineItemRefunded"]).to.eql(0);
...
# post:             pm.expect(line_item["uplineSaleType"]).to.eql("POST_WHOLESALE");
# post:            }
# post:            else if(line_item["variantInfo"]["id"] === "fa60f26f-a4c3-4895-bc22-e2638dcd10cf"){
# post:             pm.expect(line_item["upstreamLineItemDiscount"]).to.eql(0);
# post:             pm.expect(line_item["lineItemDiscount"]).to.eql(0);
# post:             pm.expect(line_item["commissionRate"]).to.eql(25);
# post:             pm.expect(line_item["commissionGross"]).to.eql(50);
# post:             pm.expect(line_item["commissionDeduction"]).to.eql(0);
# post:             pm.expect(line_item["costPrice"]).to.eql(150);
# post:             pm.expect(line_item["lineItemSubtotal"]).to.eql(200);                     
# post:             pm.expect(line_item["downlineCommissionRate"]).to.eql(0);
...
def test_lury_t2237_consumer_4_3_mixed_items_enabled_my_items_resell_promoter_retail(ctx):
    """Apifox case #7542846: Lury_T2237_Consumer_4_3_Mixed_items_enabled_my_items_resell_promoter_retail"""
    ...
    ctx.extract('merchant_access_token', _resp7, '$.data.token')
    # step 8: get merchant orders
    _resp8 = ctx.api.orders.promoter(params={'pageSize': '30', 'pageNumber': '1'})
    vars['merchantOrderId'] = 'curatorOrderId'
    # step 9: get merhcant order detail v2
    _resp9 = ctx.api.orders.promoter_detail_v2()
    # step 10: get merchant commission charge
    _resp10 = ctx.api.orders.merchant_detail_v2(path_vars={'merchantId': '{{merchantID7}}'})
    # step 11: Login - promoter
    ...
    _j = _resp12.json() if _resp12.headers.get('content-type','').startswith('application/json') else {}
    _arr = _get_path(_j, ["data", "items"]) or []
    for _order in _arr:
        if _order.get('orderNumber') == vars.get('orderNumber'):
            vars['promoterOrderId'] = _order.get('id')
    # step 13: get promoter order detail v2
    _resp13 = ctx.api.orders.promoter_detail_v2(path_vars={'merchantOrderId': '{{promoterOrderId}}'})
