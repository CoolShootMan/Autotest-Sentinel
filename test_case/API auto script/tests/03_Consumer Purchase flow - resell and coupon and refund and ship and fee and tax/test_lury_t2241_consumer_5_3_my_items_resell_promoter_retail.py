"""Migrated from Apifox case #7542927. Source folder: Consumer Purchase flow - resell and coupon and refund and ship and fee and tax."""
# Apifox Case ID: 7542927  (traceability only — not needed to run)
NAME = "_Consumer_5-3_My_items_resell_promoter_retail"
...
# pre: var transaction_fee_item_level = Math.floor(get_transaction_fee_item_level_general(80,1)*100)/100;
# pre: var promoter_transaction_fee = Math.round(((30/80 * transaction_fee_item_level) + Number.EPSILON) *100 ) / 100;
...
# post:     pm.response.to.have.status(201);
# post:     var res = JSON.parse(responseBody);
# post:     pm.expect(res['data']['totalTopay']).to.eql(84.99);
# post: });
...
# post[customScript]: pm.test("Check merchant processing fee and earnings", function () {
...
# post:     pm.expect(res['data']['totalCommissionSubtotal']).to.eql(30);
# post:     pm.expect(res['data']['totalCommissionRefunded']).to.eql(0);
# post:     pm.expect(res['data']['totalCommissionNet']).to.eql(30);
...
# post:             pm.expect(line_item["commissionRate"]).to.eql(37.5);
...
# post:             pm.expect(line_item["commissionSubtotal"]).to.eql(30);
# post:             pm.expect(line_item["commissionRefunded"]).to.eql(0);
# post:             pm.expect(line_item["commissionNet"]).to.eql(30);          
...
def test_lury_t2241_consumer_5_3_my_items_resell_promoter_retail(ctx):
    """Apifox case #7542927: Lury_T2241_Consumer_5_3_My_items_resell_promoter_retail"""
