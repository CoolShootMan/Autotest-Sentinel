"""Migrated from Apifox case #7542783. Source folder: Consumer Purchase flow - resell and coupon and refund and ship and fee and tax."""
# Apifox Case ID: 7542783  (traceability only — not needed to run)
NAME = "_Consumer_4-2_Mixed_items_enabled_my_items_resell_retail"
...
# pre: var transaction_fee_item_level = Math.floor(get_transaction_fee_item_level_general(150,1)*100)/100;
# pre: var merchant_transaction_fee = Math.round(((52.5/150 * transaction_fee_item_level) + Number.EPSILON) *100 ) / 100;
# pre: var promoter_transaction_fee = Math.round(((22.5/150 * transaction_fee_item_level) + Number.EPSILON) *100 ) / 100;
...
# post:     pm.response.to.have.status(201);
# post:     var res = JSON.parse(responseBody);
# post:     pm.expect(res['data']['totalTopay']).to.eql(309.98);
# post: });
# --- step 5: Login - P1 ---
...
# post:     pm.expect(res['data']['totalCommissionSubtotal']).to.eql(52.5);
# post:     pm.expect(res['data']['totalCommissionRefunded']).to.eql(0);
# post:     pm.expect(res['data']['totalCommissionNet']).to.eql(52.5);
...
# post:             pm.expect(line_item["commissionDeduction"]).to.eql(22.5);
...
# post:             pm.expect(line_item["commissionSubtotal"]).to.eql(52.5);
# post:             pm.expect(line_item["commissionRefunded"]).to.eql(0);
# post:             pm.expect(line_item["commissionNet"]).to.eql(52.5);          
...
# pre: var order_level_transaction_fee = Math.floor((4.99 * 0.085)*100)/100;
# pre: var transaction_fee_item_level = Math.floor(get_transaction_fee_item_level_general(150,1)*100)/100;
# pre: var promoter_transaction_fee2 = Math.round(((22.5/150 * transaction_fee_item_level) + Number.EPSILON) *100 ) / 100;
# pre: var merchant_transaction_fee = Number((transaction_fee_item_level - promoter_transaction_fee2 + order_level_transaction_fee).toFixed(2));
# pre: pm.environment.set("merchant_transaction_fee", merchant_transaction_fee);
...
# post:     pm.expect(data["totalPayoutToPay"]).to.eql(total_payout);
# post:     pm.expect(data["totalCommissionSubtotal"]).to.eql(127.5);
# post:     pm.expect(data["totalCommissionRefunded"]).to.eql(0);
# post:     pm.expect(data["totalCommissionNet"]).to.eql(127.5);
# post:     pm.expect(data["transactionFeeRebate"]).to.eql(0);
# post:     pm.expect(data["totalPaymentProcessingFee"]).to.eql(0);
# post:     pm.expect(data["totalShippingSubtotal"]).to.eql(4.99);
# post:     pm.expect(data["totalShippingRefunded"]).to.eql(0);
...
# post:     pm.expect(res['data']['totalCommissionSubtotal']).to.eql(45);
# post:     pm.expect(res['data']['totalCommissionRefunded']).to.eql(0);
# post:     pm.expect(res['data']['totalCommissionNet']).to.eql(45);
# post:     pm.expect(res['data']['totalPaymentProcessingFee']).to.eql(0);
# post:     pm.expect(res['data']['transactionFeeRebate']).to.eql(0);
# post:     var line_item_list = res['data']['lineItems'];
# post:     for (var line_item of line_item_list){
...
# post:             pm.expect(line_item["commissionGross"]).to.eql(22.5);
# post:             pm.expect(line_item["commissionDeduction"]).to.eql(0);
# post:             pm.expect(line_item["costPrice"]).to.eql(127.5);
# post:             pm.expect(line_item["lineItemSubtotal"]).to.eql(150);                     
# post:             pm.expect(line_item["downlineCommissionRate"]).to.eql(0);
# post:             pm.expect(line_item["commissionSubtotal"]).to.eql(22.5);
# post:             pm.expect(line_item["commissionRefunded"]).to.eql(0);
# post:             pm.expect(line_item["commissionNet"]).to.eql(22.5);          
# post:             pm.expect(line_item["quantity"]).to.eql(1);
# post:             pm.expect(line_item["quantityFulfilled"]).to.eql(0);
# post:             pm.expect(line_item["quantityRefunded"]).to.eql(0);
# post:             pm.expect(line_item["lineItemRefunded"]).to.eql(0);
...
def test_lury_t2236_consumer_4_2_mixed_items_enabled_my_items_resell_retail(ctx):
    """Apifox case #7542783: Lury_T2236_Consumer_4_2_Mixed_items_enabled_my_items_resell_retail"""
