"""Migrated from Apifox case #7542745. Source folder: Consumer Purchase flow - resell and coupon and refund and ship and fee and tax."""
# Apifox Case ID: 7542745  (traceability only — not needed to run)
NAME = "_Consumer_3-2_mixed_items_disabled_my_items_resell_retail"
...
# pre: 
# pre: 
# pre: var order_level_transaction_fee = Math.floor(((4.99 + 8.66) * 0.085)*100)/100;
# pre: var transaction_fee_item_level = Math.floor(get_transaction_fee_item_level_general(100,1)*100)/100;
# pre: var merchant_transaction_fee = Math.round(((15/100 * transaction_fee_item_level) + Number.EPSILON) *100 ) / 100;
# pre: var promoter_transaction_fee = Math.round(((10/100 * transaction_fee_item_level) + Number.EPSILON) *100 ) / 100;
...
# post:     pm.response.to.have.status(201);
# post:     var res = JSON.parse(responseBody);
# post:     pm.expect(res['data']['totalTopay']).to.eql(238.64);
# post: });
# --- step 5: Login - P1 ---
...
# post:     pm.expect(res['data']['totalCommissionSubtotal']).to.eql(15);
# post:     pm.expect(res['data']['totalCommissionRefunded']).to.eql(0);
# post:     pm.expect(res['data']['totalCommissionNet']).to.eql(15);
...
# post:            if (line_item["variantInfo"]["id"] === "0a080cde-22b7-457b-b843-02f1f5101769"){
...
# post:             pm.expect(line_item["commissionDeduction"]).to.eql(10);
...
# pre: var transaction_fee_item_level = Number((get_transaction_fee_item_level_general(120,1)).toFixed(2));
...
# post:     pm.expect(res['data']['totalCommissionSubtotal']).to.eql(28);
# post:     pm.expect(res['data']['totalCommissionRefunded']).to.eql(0);
# post:     pm.expect(res['data']['totalCommissionNet']).to.eql(28);
# post:     pm.expect(res['data']['totalPaymentProcessingFee']).to.eql(0);
# post:     pm.expect(res['data']['transactionFeeRebate']).to.eql(0);
# post:     var line_item_list = res['data']['lineItems'];
# post:     for (var line_item of line_item_list){
...
# post:             pm.expect(line_item["costPrice"]).to.eql(90);
# post:             pm.expect(line_item["lineItemSubtotal"]).to.eql(100);                     
...
def test_lury_t2234_consumer_3_2_mixed_items_disabled_my_items_resell_retail(ctx):
    """Apifox case #7542745: Lury_T2234_Consumer_3_2_mixed_items_disabled_my_items_resell_retail"""
    ...
    # step 6: get P1 order detail v2
    _resp6 = ctx.api.orders.merchant_detail_v2(path_vars={'merchantId': '{{P1_merchantIDa}}'})
    # step 7: Login - merchant
