"""Migrated from Apifox case #7543017. Source folder: Consumer Purchase flow - resell and coupon and refund and ship and fee and tax."""
# Apifox Case ID: 7543017  (traceability only — not needed to run)
NAME = "_Consumer_7-2_Other_items_enabled_disabled_resell_retail"
...
# post:     var total_payout = Number((res['data']['totalCommissionNet'] - promoter_transaction_fee  - promoter_transaction_fee1).toFixed(2));
# post:     pm.expect(res['data']['totalPayoutToPay']).to.eql(total_payout);
# post:     pm.expect(res['data']['transactionFee']).to.eql(promoter_transaction_fee + promoter_transaction_fee1);
# post:     pm.expect(res['data']['totalCommissionSubtotal']).to.eql(33);
# post:     pm.expect(res['data']['totalCommissionRefunded']).to.eql(0);
# post:     pm.expect(res['data']['totalCommissionNet']).to.eql(33);
# post:     pm.expect(res['data']['totalPaymentProcessingFee']).to.eql(0);
# post:     pm.expect(res['data']['transactionFeeRebate']).to.eql(0);
# post:     var line_item_list = res['data']['lineItems'];
# post:     for (var line_item of line_item_list){
...
def test_lury_t2245_consumer_7_2_other_items_enabled_disabled_resell_retail(ctx):
    """Apifox case #7543017: Lury_T2245_Consumer_7_2_Other_items_enabled_disabled_resell_retail"""
