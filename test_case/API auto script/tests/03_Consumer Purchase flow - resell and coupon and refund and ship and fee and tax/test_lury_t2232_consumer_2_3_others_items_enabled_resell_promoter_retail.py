"""Migrated from Apifox case #7542736. Source folder: Consumer Purchase flow - resell and coupon and refund and ship and fee and tax."""
# Apifox Case ID: 7542736  (traceability only — not needed to run)
NAME = "_Consumer_2-3_Others_items_enabled_resell_promoter_retail"
...
# pre: var merchant_transaction_fee = Math.round(((15/200 * transaction_fee_item_level) + Number.EPSILON) *100 ) / 100;
# pre: var promoter_transaction_fee = Math.round(((110/200 * transaction_fee_item_level) + Number.EPSILON) *100 ) / 100;
...
# post:     pm.response.to.have.status(201);
# post:     var res = JSON.parse(responseBody);
# post:     pm.expect(res['data']['totalTopay']).to.eql(204.99);
# post: });
# --- step 5: Login - P1 ---
...
# post:             pm.expect(line_item["commissionDeduction"]).to.eql(30);
# post:             pm.expect(line_item["costPrice"]).to.eql(75);
# post:             pm.expect(line_item["lineItemSubtotal"]).to.eql(120);                     
# post:             pm.expect(line_item["downlineCommissionRate"]).to.eql(55);
...
# post: });
# --- step 12: get promoter order detail v2 ---
...
# post:     pm.expect(res['data']['totalCommissionSubtotal']).to.eql(110);
# post:     pm.expect(res['data']['totalCommissionRefunded']).to.eql(0);
# post:     pm.expect(res['data']['totalCommissionNet']).to.eql(110);
# post:     pm.expect(res['data']['totalPaymentProcessingFee']).to.eql(0);
# post:     pm.expect(res['data']['transactionFeeRebate']).to.eql(0);
# post:     var line_item_list = res['data']['lineItems'];
# post:     for (var line_item of line_item_list){
...
# post:             pm.expect(line_item["subscriptionDiscount"]).to.eql(0);
# post:             pm.expect(line_item["lineItemDiscount"]).to.eql(0);
# post:             pm.expect(line_item["commissionRate"]).to.eql(55);
# post:             pm.expect(line_item["commissionGross"]).to.eql(110);
...
# post:             pm.expect(line_item["commissionSubtotal"]).to.eql(110);
# post:             pm.expect(line_item["commissionRefunded"]).to.eql(0);
# post:             pm.expect(line_item["commissionNet"]).to.eql(110);          
# post:             pm.expect(line_item["quantity"]).to.eql(1);
# post:             pm.expect(line_item["quantityFulfilled"]).to.eql(0);
# post:             pm.expect(line_item["quantityRefunded"]).to.eql(0);
# post:             pm.expect(line_item["lineItemRefunded"]).to.eql(0);
...
def test_lury_t2232_consumer_2_3_others_items_enabled_resell_promoter_retail(ctx):
    """Apifox case #7542736: Lury_T2232_Consumer_2_3_Others_items_enabled_resell_promoter_retail"""
    ...
    ctx.extract('promoter_access_token', _resp10, '$.data.token')
    # step 11: get promoter orders
    _resp11 = ctx.api.orders.promoter(params={'pageSize': '30', 'pageNumber': '1'})
    _j = _resp11.json() if _resp11.headers.get('content-type','').startswith('application/json') else {}
    _arr = _get_path(_j, ["data", "items"]) or []
    for _order in _arr:
        if _order.get('orderNumber') == vars.get('orderNumber'):
            vars['promoterOrderId'] = _order.get('id')
    # step 12: get promoter order detail v2
    _resp12 = ctx.api.orders.promoter_detail_v2(path_vars={'merchantOrderId': '{{promoterOrderId}}'})
