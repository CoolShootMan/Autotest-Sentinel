"""Migrated from Apifox case #8602156. Source folder: Event and ticket and redeemed/Reveal address."""
# Apifox Case ID: 8602156  (traceability only — not needed to run)
NAME = "(Lury)Resell Verify reveal address before at time check on order confirmation page"
...
# post:         if (line_item["merchantProductId"] === "6708f6d2-d45f-41b9-a3c8-4e743bfcba2b"){
# post:             pm.expect(line_item["extraInfo"]["isAddressHidden"]).to.eql(true);
# post:             pm.expect(line_item["extraInfo"]["location"]).to.eql("");
# post:             pm.expect(line_item["extraInfo"]["event"]).to.eql("Lury event reveal address before at automation testing 0806");
# post:             pm.expect(line_item["extraInfo"]["venue"]).to.eql("reveal address before at 2035-8-6");
# post:         }
# post:     }
# post: 
# post: });
...
def test_lury_resell_verify_reveal_address_before_at_time_check_on_order_confirmation_page(ctx):
    """Apifox case #8602156: Lury_Resell_Verify_reveal_address_before_at_time_check_on_order_confirmation_pag"""
    ...
    # step 3: buy now
    _resp3 = ctx.api.orders.checkout_express(body='{\n    "items": [\n        {\n            "quantity": 1,\n            "promoterProductVariantId": "b7dc7778-3989-4573-98f5-dc201cd7d86d",\n            "price": 0,\n            "postId": "ea2dce54-3ab2-4473-bc8d-c74fe449beb2",\n            "selected": true,\n            "customFields": []\n        }\n    ],\n    "fbAdParams": {\n        "eventID": "35ea07ac-6896-4fb0-a0ca-e90a363b8c04",\n        "pixelId": [\n            "268933192948110"\n        ],\n        "fbBrowserId": "fb.1.1768467567031.92506180232323161",\n        "externalId": "154873ad-8fbe-4f69-bdf0-e11205c4eab8",\n        "eventSourceUrl": "https://release.pear.us/auto-merchant/post/lury-event-reveal-address-before-at-automation-testing-0806"\n    },\n    "subdomainVanityUrl": ""\n}')
