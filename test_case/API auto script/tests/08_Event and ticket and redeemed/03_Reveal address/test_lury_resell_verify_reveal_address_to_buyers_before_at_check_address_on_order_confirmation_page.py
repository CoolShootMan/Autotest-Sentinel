"""Migrated from Apifox case #8602229. Source folder: Event and ticket and redeemed/Reveal address."""
# Apifox Case ID: 8602229  (traceability only — not needed to run)
NAME = "(Lury)Resell Verify reveal address to buyers before at check address on order confirmation page"
...
# post:         if (line_item["merchantProductId"] === "a8ee1a82-0f62-4bf2-874b-9e6447a67844"){
# post:             pm.expect(line_item["extraInfo"]["isAddressHidden"]).to.eql(true);
# post:             pm.expect(line_item["extraInfo"]["location"]).to.eql("");
# post:             pm.expect(line_item["extraInfo"]["event"]).to.eql("Lury event reveal address to buyer before at automation testing 0806");
# post:             pm.expect(line_item["extraInfo"]["venue"]).to.eql("reveal address to buyers before at 2026-8-6");
# post:         }
# post:     }
# post: 
# post: });
...
def test_lury_resell_verify_reveal_address_to_buyers_before_at_check_address_on_order_confirmation_page(ctx):
    """Apifox case #8602229: Lury_Resell_Verify_reveal_address_to_buyers_before_at_check_address_on_order_con"""
    ...
    # step 3: buy now
    _resp3 = ctx.api.orders.checkout_express(body='{\n    "items": [\n        {\n            "quantity": 1,\n            "promoterProductVariantId": "b33de999-4be9-46df-abf4-7e97e281ffb6",\n            "price": 0,\n            "postId": "3aa86982-0f13-4c39-8c1a-c132301daef4",\n            "selected": true,\n            "customFields": []\n        }\n    ],\n    "fbAdParams": {\n        "eventID": "cc55923d-6062-44b5-8a62-c8f69d848d81",\n        "pixelId": [\n            "268933192948110"\n        ],\n        "fbBrowserId": "fb.1.1768467567031.92506180232323161",\n        "externalId": "154873ad-8fbe-4f69-bdf0-e11205c4eab8",\n        "eventSourceUrl": "https://release.pear.us/auto-merchant/post/lury-event-reveal-address-to-buyer-before-at-automation-testing-0806"\n    },\n    "subdomainVanityUrl": ""\n}')
