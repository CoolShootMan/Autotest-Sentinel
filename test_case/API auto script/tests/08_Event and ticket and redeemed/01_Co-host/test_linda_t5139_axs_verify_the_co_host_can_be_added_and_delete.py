"""Migrated from Apifox case #8709242. Source folder: Event and ticket and redeemed/Co-host."""
# Apifox Case ID: 8709242  (traceability only — not needed to run)
NAME = "(AXS)Verify the Co-host can be added and delete"
...
# --- step 1: 8267b0e0-c9c0-42ad-ba59-ed4da50cdf81 ---
...
# post: }
# --- step 2: 5f545808-d616-42d8-a257-3d633040c77f ---
...
# post: });
# --- step 3: e6f5ac94-1723-4f62-b5fb-8f75d31552a8 ---
# post[customScript]: // 1. 解析响应 JSON
# post: const responseData = pm.response.json();
# post: 
# post: // 2. 获取环境变量中的预期值
...
# post: });
# --- step 4: fd8c8613-31ec-4a30-9a43-e67700f9b1c4 ---
# post[customScript]: // 1. 解析响应 JSON
# post: const responseData = pm.response.json();
# post: 
...
def test_linda_t5139_axs_verify_the_co_host_can_be_added_and_delete(ctx):
    """Apifox case #8709242: Linda_T5139_AXS_Verify_the_Co_host_can_be_added_and_delete"""
    ...
    _t1 = next((it for it in (_j1.get('data') or {}).get('items') or [] if it.get('platform') == 'AXS'), None)
    assert _t1 is not None, "未找到 platform 为 AXS 的项目"
